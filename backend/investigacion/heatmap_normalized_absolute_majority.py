...
votes_prediction = np.zeros((height_image,width_image,4), dtype=np.uint8)
...
for _, row in df_slide.iterrows():
    ...
    if os.path.exists(patch_path):
        ...
        if prediction in [0,1,2,3]:
            votes_slice_prediction = votes_prediction[y_image:final_y, x_image:final_x, prediction]
            votes_slice_prediction[mask] += 1


heatmap_ground_truth = votes(votes_ground_truth,width_image, height_image, colours ,alpha)
heatmap_prediction = votes(votes_prediction,width_image, height_image, colours ,alpha)

original_img = background_image.convert('RGB')
original_img.thumbnail((5000,5000), Image.Resampling.LANCZOS)
output = f"Absolute_majority_normalized_heatmap_{slide}_{features_extractor}_{model_classifier}_Original.jpg"
...
output = f"Absolute_majority_normalized_heatmap_{slide}_{features_extractor}_{model_classifier}_GroundTruth.jpg"
...
output = f"Absolute_majority_normalized_heatmap_{slide}_{features_extractor}_{model_classifier}_Prediction.jpg"

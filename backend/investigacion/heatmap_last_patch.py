...
features_val = "./features/val/features_virchow2val.npy"
alpha = 160
threshold = 200
colours = {
    1: (255,255,0),
...
def apply_alpha(img_rgba, target_alpha):
    r,g,b,a = img_rgba.split()
    a = a.point(lambda p:target_alpha if p > 0 else 0)
    img_rgba.putalpha(a)
    return img_rgba
...
scaler_path = f"./trained_models/scaler_{features_extractor}.joblib"
model_path = f"./trained_models/{model_classifier}_{features_extractor}.keras"
...
heatmap_ground_truth = Image.new('RGBA', (width_image, height_image), (255,255,255,0))
heatmap_prediction = Image.new('RGBA', (width_image, height_image), (255,255,255,0))
...
for _, row in df_slide.iterrows():
    ...
    if os.path.exists(patch_path):
        ...
        masks[name] = tissue_mask

df_ground_truth = df_slide.sort_values(by='label', ascending=True)
for _, row in df_ground_truth.iterrows():
    ...
    if name in masks:
        ...
        if color_ground_truth:
            ...
            heatmap_ground_truth.paste(color_image_ground_truth, (x_image, y_image), masks[name])

df_prediction = df_slide.sort_values(by='prediction', ascending=True)
for _, row in df_prediction.iterrows():
    ...
    if name in masks:
        ...
        if color_prediction:
            ...
            heatmap_prediction.paste(color_image_prediction, (x_image, y_image), masks[name])

heatmap_ground_truth = apply_alpha(heatmap_ground_truth, alpha)
heatmap_prediction = apply_alpha(heatmap_prediction, alpha)

original_img = background_image.convert('RGB')
original_img.thumbnail((5000,5000), Image.Resampling.LANCZOS)
output = f"NUEVO5_heatmap_{slide}_{features_extractor}_{model_classifier}_Original.jpg"
...
output = f"NUEVO5_heatmap_{slide}_{features_extractor}_{model_classifier}_GroundTruth.jpg"
...
output = f"NUEVO5_heatmap_{slide}_{features_extractor}_{model_classifier}_Prediction.jpg"

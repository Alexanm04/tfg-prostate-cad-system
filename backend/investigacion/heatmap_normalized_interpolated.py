...
def normalize_alpha(img, target):
    r,g,b,a = img.split()
    a = a.point(lambda p: target if p > 0 else 0)
    img.putalpha(a)
    return img
...
df_slide = df_slide.sort_values(by='prediction', ascending=True)
...
heatmap_ground_truth = Image.new('RGBA', (width_image, height_image), (0,0,0,0))
heatmap_prediction = Image.new('RGBA', (width_image, height_image), (0,0,0,0))
...
for _, row in df_slide.iterrows():
    ...
    if os.path.exists(patch_path):
        ...
        tissue_mask = Image.fromarray((mask * 255).astype(np.uint8), 'L')
        alpha_mask = tissue_mask.point(lambda p: alpha if p > 0 else 0)
        box = (x_image, y_image, x_image + patch_size, y_image + patch_size)
        color_ground_truth = colours.get(real_label)

        if color_ground_truth:
            color_image_ground_truth = Image.new('RGBA', (patch_size, patch_size), color_ground_truth + (255,))
            color_image_ground_truth.putalpha(alpha_mask)
            background_crop = heatmap_ground_truth.crop(box)
            blended_ground_truth = Image.alpha_composite(background_crop, color_image_ground_truth)
            heatmap_ground_truth.paste(blended_ground_truth, box)

        color_prediction = colours.get(prediction)
        if color_prediction:
            color_image_prediction = Image.new('RGBA', (patch_size, patch_size), color_prediction + (255,))
            color_image_prediction.putalpha(alpha_mask)
            background_crop = heatmap_prediction.crop(box)
            blended_prediction = Image.alpha_composite(background_crop, color_image_prediction)
            heatmap_prediction.paste(blended_prediction, box)

heatmap_ground_truth = normalize_alpha(heatmap_ground_truth, alpha)
heatmap_prediction = normalize_alpha(heatmap_prediction, alpha)

original_img = background_image.convert('RGB')
original_img.thumbnail((5000,5000), Image.Resampling.LANCZOS)
output = f"NNuevo1_Interpolated_normalized_heatmap_{slide}_{features_extractor}_{model_classifier}_Original.jpg"
...
output = f"NNuevo1_Interpolated_normalized_heatmap_{slide}_{features_extractor}_{model_classifier}_GroundTruth.jpg"
...
output = f"NNuevo1_Interpolated_normalized_heatmap_{slide}_{features_extractor}_{model_classifier}_Prediction.jpg"

...
def add_legend(img, healthy_patch):
    ...
    rect_size = int(320 * scale)
    spacing = int(100 * scale)
    margin = int(100 * scale)
    outline_w = max(4, int(15*scale))
...
def binary_votes(votes_matrix, width, height, colours, alpha):
    winner = np.zeros((height, width), dtype= np.uint8)
    more_cancer = votes_matrix[:,:,1] >= votes_matrix[:,:,0]
    votes = (votes_matrix[:,:,0] + votes_matrix[:,:,1]) > 0
    winner[more_cancer & votes] = 1
    heatmap = np.zeros((height, width,4), dtype= np.uint8)
    color = colours[1]
    heatmap[winner == 1] = (color[0], color[1], color[2], alpha)

    return Image.fromarray(heatmap, 'RGBA') 

def binary_cwca(score_matrix, width, height, colours, alpha):
    healthy_votes = score_matrix[:,:,0]
    cancer_votes = score_matrix[:,:,1]
    winner = np.where(cancer_votes>=healthy_votes, 1, 0)
    valid_mask = (healthy_votes + cancer_votes) > 0

    heatmap = np.zeros((height, width, 4), dtype=np.uint8)
    color = colours[1]
    heatmap[(winner == 1) & valid_mask] = (color[0], color[1], color[2], alpha)
...
y_cancer_probability =y_probability[:,1:].sum(axis=1)
y_binary_probability = np.stack([1-y_cancer_probability, y_cancer_probability], axis=1)
binary_predictions = (y_cancer_probability > 0.5).astype(int)
df_slide['prediction'] = binary_predictions
df_slide['binary_label'] = binary(df_slide['label'].values)

binary_probabilities_safe = np.clip(y_binary_probability, 1e-9, 1.0)
entropies = -np.sum(binary_probabilities_safe * np.log2(binary_probabilities_safe), axis=1)
...
votes_ground_truth = np.zeros((height_image,width_image,2), dtype=np.uint8)
cwca_scores = np.zeros((height_image,width_image,2), dtype=np.float32)

healthy_patch = None
max_pixels = -1
for idx, (original_idx, row) in enumerate(df_slide.iterrows()):
    name = row['image_name']
    real_label = row['binary_label']
    x, y = extract_coordinates(name)
    ...
    if os.path.exists(patch_path):
        ...
        if real_label in [0,1]:
            votes_ground_truth[y_image:final_y, x_image:final_x, real_label][mask] += 1
        
        patch_weight = cwca_weights[idx]
        for c in [0,1]:
            cwca_scores[y_image:final_y, x_image:final_x, c][mask] += (patch_weight * binary_probabilities_safe[idx,c])

heatmap_ground_truth = binary_votes(votes_ground_truth,width_image, height_image, colours ,alpha)
heatmap_prediction = binary_cwca(cwca_scores,width_image, height_image, colours ,alpha)

original_img = background_image.convert('RGB')
original_img.thumbnail((5000,5000), Image.Resampling.LANCZOS)
output = f"Binary_Cwca_normalized_heatmap_{slide}_{features_extractor}_{model_classifier}_Original.jpg"
...
output = f"Binary_Cwca_normalized_heatmap_{slide}_{features_extractor}_{model_classifier}_GroundTruth.jpg"
...
output = f"Binary_Cwca_normalized_heatmap_{slide}_{features_extractor}_{model_classifier}_Prediction.jpg"
